...
describe "Form For" do
  ...
  context "when a form has errors" do
    ...
    it "should show error around relevant form item" do
      ...
      expected = '<div class="errorExplanation" id="errorExplanation"><h2>1 error occurred while processing information</h2><ul><li>login - login already used</li></ul></div><form method="POST" class="tester" action="' + (object_url @user_drop) + '"><input type="hidden" name="_method" value="PUT"/><span class="fieldWithErrors"><label for="liquid_demo_model_login">Login</label></span><span class="fieldWithErrors"><input id="liquid_demo_model_login" name="liquid_demo_model[login]" type="text" value="' + @user_drop[:login] + '" /></span></form>'
